...
key="123456789abcdefghijkl"
maximums={"rain.1h":1,"temp":25} # whatever keys are available here: https://openweathermap.org/api/one-call-api#parameter , we'll take the boolean
...
def main():
	...
	# step 3, process data, checking each location and hour.
	...
	for l,(lat,lon) in enumerate(locations):
		...
		for hour in hours:
			# 3.a: which hour are we looking at? and so on
			...
			# 3.b psuedo-flatten hour: any values which themselves are dicts, turn into key.subkey=value
			for key in list(hour.keys()):					# eg "rain": {"1h": 0.14}
				if type(hour[key])==dict:
					for k in hour[key].keys():		# "1h" from above
						hour[key+"."+k]=hour[key][k]	# turns into "rain.1h"
			
			# 3.c time to get down to business	
